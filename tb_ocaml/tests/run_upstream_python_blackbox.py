...
TB_OCAML_ROOT = resolve_path(
    "TB_OCAML_ROOT",
    Path(__file__).resolve().parents[1],
    base=Path.cwd(),
)
REPO_ROOT = TB_OCAML_ROOT.parent
OCAML_BIN = resolve_path(
    "TB_OCAML_BIN",
    TB_OCAML_ROOT / "_build/default/bin/tigerbeetle.exe",
    base=TB_OCAML_ROOT,
)
OCAML_CHECKSUM_BIN = resolve_path(
    "TB_OCAML_CHECKSUM_BIN",
    TB_OCAML_ROOT / "_build_tools/tb_checksum",
    base=TB_OCAML_ROOT,
)
UPSTREAM_PY_TEST = REPO_ROOT / "repo/src/clients/python/tests/test_basic.py"
DEFAULT_VENV_ACTIVATE = REPO_ROOT / ".venv_pytests/bin/activate"
