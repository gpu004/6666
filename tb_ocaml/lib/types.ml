...
let event_size = function
  ...
  | op
    when op = op_deprecated_lookup_accounts_unbatched
         || op = op_deprecated_lookup_transfers_unbatched ->
      id_size
  | op
    when op = op_deprecated_get_account_transfers_unbatched
         || op = op_deprecated_get_account_balances_unbatched ->
      account_filter_size
  | op
    when op = op_deprecated_query_accounts_unbatched
  ...
  | op when op = op_get_account_transfers || op = op_get_account_balances ->
      account_filter_size
  | op when op = op_query_accounts || op = op_query_transfers ->
      query_filter_size
...
let result_size = function
  ...
  | op
    when op = op_deprecated_lookup_accounts_unbatched
         || op = op_deprecated_query_accounts_unbatched ->
      account_size
  | op
    when op = op_deprecated_lookup_transfers_unbatched
         ...
         || op = op_deprecated_query_transfers_unbatched ->
      transfer_size
  | op when op = op_deprecated_get_account_balances_unbatched ->
      account_balance_size
  | op
    when op = op_deprecated_create_accounts_sparse
         || op = op_deprecated_create_transfers_sparse ->
      create_error_result_size
  | op when op = op_create_accounts || op = op_create_transfers ->
      create_result_size
  | op when op = op_lookup_accounts || op = op_query_accounts -> account_size
  | op
    when op = op_lookup_transfers
...
let is_multi_batch = function
  | op
    when op = op_deprecated_create_accounts_sparse
         ...
         || op = op_lookup_accounts || op = op_lookup_transfers
         || op = op_get_account_transfers
         || op = op_get_account_balances
         || op = op_query_accounts || op = op_query_transfers ->
      true
  | _ -> false

let account_status_name = function
  | s when s = create_account_created ->
...
let transfer_status_name = function
  ...
  | s when s = create_transfer_exists ->
...
let transfer_flag_names flags =
  ...
  if bool_flag flags transfer_flag_closing_debit then
    out := "closing_debit" :: !out;
  if bool_flag flags transfer_flag_closing_credit then
    out := "closing_credit" :: !out;
